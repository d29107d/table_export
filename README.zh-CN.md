...
单元格里填**值本身**就行，需要的 Lua 语法（长字符串外框、引号）由导出器补上，
**不需要自己写 `[[ ]]`**。

| 类型 | 单元格里填什么 | 产物里长什么样 | 说明 |
|---|---|---|---|
| `number` | `12` | `12` | 转不成数字的值会被**静默写成 nil**（数据丢失），所以这里会**直接报错**而不是放过。 |
| `string` | `长剑` —— 纯文本，**不要**自己加 `[[ ]]` | `[[长剑]]` | 外框由导出器补上，内容不用转义。若内容含 `]`，会自动提升括号层级（`[[` → `[=[` → `[==[` …），保证产物仍是合法 Lua。 |
| `table` | `{{1001, 2}, {1002, 1}}` | 与填写内容一致 | Lua 表格由你自己写，内容会做**语法校验**。 |
| `any` | `nil` / `true` / `100+50` / `{quality=3}` | 与填写内容一致 | 同 `table`，另外裸数字与布尔值也直接透传。 |

> **`string` 单元格请只填纯文本。** 自己加上 `[[ ]]` 的话，它们会变成值的一部分：
> 导出器依然会正确转义，但导出的字符串里会真的带上这对括号。
