...
- **先校验再写文件**：`table` / `any` 列会被当作 Lua 表达式做语法检查，`number` 列
  会检查是否真能转成数字，**key 被两行及以上重复使用**也会报出来（这种会静默丢数据）。
  **只要有一处不对，导出直接中断、一个文件都不写**，绝不会留下"导了一半"的目录。
- **多项目管理**：可保存多套「源目录 + 输出目录」组合并随时切换，全部记在
  `config/projects.json`。
- **SVN 辅助**：「更新表格」「提交表格」「提交前端」「提交后端」按钮 —— 一份工作副本
  一个提交按钮，表格和导出产物各自独立（Windows 走 TortoiseSVN 窗口，macOS 走终端
  里的 `svn`）。
...
`Ctrl+Shift+A` 反选 · `F5` 刷新 · `Ctrl+F` 定位到搜索框 · `Ctrl+S` 保存配置 ·
`Ctrl+Q` 退出。鼠标停在按钮上会显示它对应的快捷键。
macOS 上除 `F5` 外都额外绑了 `⌘`（`⌘E`、`⌘⇧E`、`⌘A`、`⌘⇧A`、`⌘F`、`⌘S`、`⌘Q`），
原来那套 `Ctrl` 保留，两种习惯都能用。
...
第 5 行是表头。字段区**遇到第一个空行就结束**：空行以下一律不导入，哪怕那些单元格
看起来还是字段。若第 6 行本身就是空行，整张表没有字段，不会生成文件。规则与 `base`
一致 —— 第 9 行为空则不导出，第 9 行起的第一个空行处表格结束。产物是一张扁平的表：
...
key 列，在客户端文件里照样当 key 用。同一条 key 被两行及以上用到时，只有最后一行
会留下（这就是 Lua 表赋值的语义），先出现的行会从产物里消失 —— 所以这被当作**数据
错误**，会**直接中断导出**（见[数据校验](#数据校验)）。
...
- **`tiny` 表遇到空行就结束**：字段区在第 6 行之后第一个空行处截断，写在空行以下的
  内容一律不导入 —— 哪怕它看起来仍然是字段。第 6 行就是空行时，整表没有字段，不会
  生成文件。这条与 `base` 的「第 9 行起的第一个空行处结束」是同一个规则。
...
带 key 的表还会检查 **key 是否重复**。两行用同一个 key 不是语法错误，危险恰恰在这里：
生成的表只留后写的那行，而导出会报告"成功"。判重比的是 **key 实际写进 lua 的字面量**，
所以 `number` 列里的 `1` 和 `1.0` 会算撞车，key 单元格空着的两行也会（都写成 `0`）。
每条报错都会指出是哪个 key 列、以及哪几行会被丢掉。
...
| `03_tiny_config.xlsx` | `tiny` 表布局 —— 一份文件一条记录，中间不留空行 |
| `04_edge_cases.xlsx` | 多行文本、含 `]]` 的文本、以 `]` 结尾的文本、"空着"与"只有空格"的对比、布尔值、科学计数、自定义文件头尾，以及 `tiny` 的两种空行情形（`EarlyStop` 在空行处截断，`NoFirstRow` 不生成文件） |
...
- 产物是纯文本，重复导出会直接覆盖。
- **写文件是全有或全无**：先在内存里把内容编码好，再用原子替换落盘，所以写失败时旧文件
  原样保留。如果某张表里有当前编码存不下的字符（例如 GB2312 存不了 `·`），报错会点名
  **哪张表、产出哪个文件、是哪个字符**，不再甩一串 codec 错误，也不会把文件截断。
- 输出编码只保留 `utf-8` 和 `gbk` 两种。`gbk` 覆盖简体中文游戏数据；下游要求 UTF-8 时
  再选 `utf-8`。
